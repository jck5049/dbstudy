/*
    ...
    1. 오라클 전용 문법이고 프로그래밍 처리가 가능한 SQL이다.(쿼리문이다)
    2. 형식
          [DECLARE
              변수 선언]
          BEGIN
...
-- EMPLOYEE_ID가 150인 사원의 COMMISSION_PCT가 0DLAUS '커미션없음', 아니면 실제 커미션(COMMISSION_PCT * SALARY)을 출력하시오.
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
...
-- EMPLOYEE_ID가 150인 PHONE_NUMBER에 따른 지역을 출력하시오.
-- 011 : MOBILE
-- 515 : EAST
-- 590 : WEST
-- 603 : SOUTH
...
BEGIN
    FOR N IN 1..10 LOOP
        ...
        END IF;
    END IF;
    DBMS_OUTPUT.PUT_LINE(N|| '은(는) ' || MESSAGE || '합니다.');
    END LOOP;
END;
...
EXCEPTION
    WHEN TOO_MANY_ROWS THEN     -- 예외가 2개 이상일때 TOO_MANY_ROWS 이다.
