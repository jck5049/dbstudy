...
-- SELECT 사원정보
--   FROM 사원테이블
--  WHERE 직급 = (사원번호가 1001인 사원의 직급); 풀이 방식을 한글로 적은 내용!
...
SELECT EMP_NO, NAME, POSITION, SALARY
  FROM EMPLOYEE_TBL
...
SELECT EMP_NO, NAME, POSITION, HIRE_DATE
  FROM EMPLOYEE_TBL
...
SELECT EMP_NO, DEPART, NAME, POSITION
  FROM EMPLOYEE_TBL
...
-- 서브쿼리를 이용한 풀이
SELECT EMP_NO, DEPART, NAME, POSITION, GENDER, HIRE_DATE, SALARY
  FROM EMPLOYEE_TBL
...
-- 참고. 조인으로 풀기
SELECT D.DEPT_NO, D.DEPT_NAME, D.LOCATION, E.POSITION
  FROM DEPARTMENT_TBL D INNER JOIN EMPLOYEE_TBL E
    ON D.DEPT_NO = E.DEPART
 WHERE POSITION = '과장';
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWID = 'AAAG/4AABAAALJxAAA';
