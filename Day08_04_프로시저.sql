...
-- 프로시저1 정의(만들기)
CREATE OR REPLACE PROCEDURE PROC1   -- 괄호 생략 가능 매개변수 없는 코드
IS
BEGIN
...
CREATE SEQUENCE BUY_SEQ
    NOCACHE;    -- 번호표를 미리 막 뽑아놓은 상태 좀더 빠르게 하기위해 사용함(미리 뽑아놓은 번호가 날아갈 수 있는점 참고!)
...
BEGIN
    ...
    INSERT INTO BUY_TBL(B_NO, C_NO, P_CODE, B_AMOUNT) VALUES(BUY_SEQ.NEXTVAL, CNO, PCODE, BUY_AMOUNT);
     ...
     -- 3) 고객 테이블의 포인트를 수정(UPDATE)한다.
     --    총 구매액의 10%를 정수로 올림 처리해서 포인트로 준다.
     -- CEIL(가격 * 구매수량 * 0.1) 포인트 10% 처리하는 방법.
     -- 위 가격은 SELECT로 처리!
...
-- 문제가 발생할 수 있으니 EXCEPTION 처리를 해두자.
EXCEPTION
