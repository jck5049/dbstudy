...
-- 기본키 제거하기 (추후에 현업에서는 제거하는건 사용 거의 안한다. 이유는 필요가 없다면 테이블을 제거하기 때문에 한번에 삭제된다.) 
-- 방법 1.
...
-- 외래키 제거하기 (추후에 현업에서는 제거하는건 사용 거의 안한다. 이유는 필요가 없다면 테이블을 제거하기 때문에 한번에 삭제된다.)
ALTER TABLE PLAYER_TBL
...
ALTER TABLE SCHEDULE_TBL
    ADD CONSTRAINT FK_SCHEDULE_NATION FOREIGN KEY(N_CODE)
        REFERENCES NATION_TBL(N_CODE)
            ON DELETE SET NULL;     -- (NULL 인 경우에) ON DELETE CASCADE도 가능하다.
