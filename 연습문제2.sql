...
CREATE SEQUENCE CUSTOMER_SEQ START WITH 1000 NOCACHE;
...
INSERT INTO CUSTOMER_TBL(CUSTOMER_ID, CUSTOMER_NAME, ADDRESS, PHONE) VALUES(CUSTOMER_SEQ.NEXTVAL, '장미랑', '대한민국', '222-222-222');
...
INSERT INTO ORDER_TBL(ORDER_ID, CUSTOMER_ID, BOOK_ID, AMOUNT, ORDER_DATE) VALUES(TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL, 1000, 3, 2, '20/07/02');
...
INSERT INTO ORDER_TBL(ORDER_ID, CUSTOMER_ID, BOOK_ID, AMOUNT, ORDER_DATE) VALUES(TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL, 1000, 2, 5, '20/07/06');
...
-- 고객명  구매도서수
-- 김연아  2
       SELECT C.CUSTOMER_NAME AS 고객명, SUM(O.AMOUNT) AS 구매도서수
         FROM CUSTOMER_TBL C
...
SELECT * FROM CUSTOMER_TBL; --김연아 CUSTOMER_ID = 1001
...
-- 고객명
-- 박세리
SELECT C.CUSTOMER_NAME AS C_고객명
  FROM CUSTOMER_TBL C 
  LEFT OUTER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID
 GROUP BY C.CUSTOMER_NAME, C.CUSTOMER_ID, O.AMOUNT
 HAVING O.AMOUNT IS NULL;
 
 
 SELECT C.CUSTOMER_NAME AS 고객명 
   FROM CUSTOMER_TBL C 
   WHERE NOT EXISTS(SELECT * FROM ORDER_TBL O WHERE O.CUSTOMER_ID=C.CUSTOMER_ID);


-- 9. '박지성'이 구매한 도서를 발간한 출판사(PUBLISHER) 개수를 조회하시오.
-- 고객명  출판사수
-- 박지성  3
SELECT C.CUSTOMER_NAME AS 고객명, COUNT(B.PUBLISHER) AS 출판사수
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
 WHERE C.CUSTOMER_NAME LIKE '%지성'
 GROUP BY C.CUSTOMER_NAME
 ;
 
 SELECT C.CUSTOMER_NAME AS 고객명, COUNT(B.PUBLISHER) AS 출판사수
   FROM BOOK_TBL B
  INNER JOIN ORDER_TBL O ON O.BOOK_ID=B.BOOK_ID
  INNER JOIN CUSTOMER_TBL C ON O.CUSTOMER_ID=C.CUSTOMER_ID
...
;
SELECT TO_DATE(ORDER_DATE,'YY/MM/DD') FROM ORDER_TBL;
...
-- 박지성  116000
-- 추신수  86000
SELECT C.CUSTOMER_NAME AS 고객명, SUM(PRICE * AMOUNT) AS 총구매액
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
...
-- 추신수  86000      2
-- 박세리  0          0
SELECT C.CUSTOMER_NAME AS 고객명, NVL(SUM(PRICE * AMOUNT), 0) AS 총구매액, COUNT(AMOUNT) AS 구매횟수
