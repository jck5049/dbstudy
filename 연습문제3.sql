...
-- 구매-사용자 외래키
ALTER TABLE BUYS    -- USER_NO가 PK이여서 외래키여야 하는데 USER_ID를 한 이유는 UNIQUE이기 때문에 가능하다.
...
DELETE  
  FROM BUYS         -- 서브쿼리를 이용해 먼저 사용자번호가 5인 사용자 구매내역부터 삭제함.
...
SELECT USER_NO, USER_ID, USER_MOBILE1, USER_MOBILE2
  FROM USERS
 WHERE USER_MOBILE2 LIKE '5%'; -- 와일드카드형이라 =이 아니고 LIKE로 사용해야한다.


-- 6. 제품을 구매한 사용자의 아이디별 구매횟수를 조회하시오.
-- 아이디  구매횟수           동일한 ID는 하나로 모아서 조회하라는 말! (GROUP BY 활용)
-- KHD     3                  GROUP BY 절에는 항상 조회할 칼럼이 들어가야한다! USER_ID를 꼭 넣어야함.
-- PSH     3
-- KYM     1
-- LHJ     2
...
-- 아이디  고객명  구매횟수  총구매액    테이블 2개가 필요함 조인으로 풀어야함.(고객명이 USERS에 있음.)
-- LHJ     이휘재  2         80            GROUP BY도 활용해야 함.(구매횟수, 총구매액)
-- KYM     김용만  1         200
-- KHD     강호동  3         1210
-- PSH     박수홍  3         1860
...
--     판매 이력이 없는 제품은 0으로 조회하시오.(외부조인 활용)
-- 제품명  판매횟수
-- 책      1
-- 메모리  1
-- 운동화  2
