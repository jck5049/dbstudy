...
-- (ANSI) 평균 JOIN 전용 문법 (드라이빙 테이블(DEPARTMENT_TBL)에 PK가 설정된 테이블이 오는게 좋다!)
-- 드리븐 테이블(EMPLOYEE_TBL)은 FK를 가진 테이블이 오는게 좋고 드라이빙 테이블 뒤에 오는 테이블이다.
...
-- 제약조건이름 : FK_EMPLOYEE_DEPART
ALTER TABLE EMPLOYEE_TBL
    DISABLE CONSTRAINT FK_EMPLOYEE_DEPART;
...
-- 외래키 제약 조건의 활성화(다시 시작)
-- 제약조건이름 : FK_EMPLOYEE_DEPART
ALTER TABLE EMPLOYEE_TBL
    ENABLE CONSTRAINT FK_EMPLOYEE_DEPART;
